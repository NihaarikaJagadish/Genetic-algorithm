...
class Individual(object):
    ...
    def cal_fitness(self):
        ...
        pix = img.load()
        loss = 0
        originalImage = Image.open('fin.jpg')
        originalPixel = originalImage.load()
        for i in range(0,32):
            for j in range(0,32):
                try:
                    diffRed   = abs(originalPixel[i,j]   - pix[i,j][0])
                    ...
                    diffRed   = abs(originalPixel[i,j][0]   - pix[i,j][0])
...
def main():
    ...
    while not found:
        ...
        if(generation % 100 == 0 or generation == 1):
            ...
            img = Image.new("RGB", (w, h),"white")
            img1 = ImageDraw.Draw(img,'RGBA') 
            for eachShape in population[0].chromosome:
                img1.rectangle(eachShape, (105,105,105,125))
            img.show()
